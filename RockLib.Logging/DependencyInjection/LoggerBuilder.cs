...
public class LoggerBuilder : ILoggerBuilder
{
    ...
    private static bool IsEmpty(LoggerOptions options) => options.LogProviderRegistrations.Count == 0 && options.ContextProviderRegistrations.Count == 0
            && options.Level is null && options.IsDisabled is null;
}